...
<style type="text/css">
	...
	#wgn_metabox td, #wgn_metabox th {padding-top: 0 !important;}
	#wgn_box_container * {margin-top: 0 !important;}
	.wgn-file-container {margin-top: 10px;}
	#wgn-file-details, #wgn-file-icon {display: inline-block;vertical-align: top;}
	#wgn-file-details {margin: 0 12px;}
	#wgn-file-details span {color:royalblue;}
	...
	@media screen and (max-width: 1215px){
		textarea#wgn_channel_pattern {width: 98%;}
		div#output {width: 98%;}
	}
	@media screen and (max-width: 782px) {
...
<tr>
	<th scope="row">
		<h3><?php echo __("Message Pattern", "wgn-plugin"); ?></h3>
	</th>
	<td>
		<p class="howto">
			<?php 
			echo __("Define the structure of messages that are sent to the channel. Use tags, emoji shortnames and any other text.", "wgn-plugin");
			?>
		</p>
		<div class="toolbar">
			<select id="patterns-select">
			<option selected="true" style="display:none;"><?php echo __("Select a tag...", "wgn-plugin"); ?></option>
				<optgroup label="WordPress Tags">
					<option title='<?php echo __("The ID of this post", "wgn-plugin"); ?>'>{ID}</option>
					<option title='<?php echo __("The title of this post", "wgn-plugin"); ?>'>{title}</option>
					<option title='<?php echo __("The first 55 words of this post", "wgn-plugin"); ?>'>{excerpt}</option>
					<option title='<?php echo __("The whole content of this post", "wgn-plugin"); ?>'>{content}</option>
					<option title='<?php echo __("The display name of author of this post", "wgn-plugin"); ?>'>{author}</option>
					<option title='<?php echo __("The short url of this post", "wgn-plugin"); ?>'>{short_url}</option>
					<option title='<?php echo __("The permalink of this post", "wgn-plugin"); ?>'>{full_url}</option>
					<option title='<?php echo __("The tags of this post. Tags are automatically converted to Gap hashtags", "wgn-plugin"); ?>'>{tags}</option>
					<option title='<?php echo __("The categories of this post. Categories are automatically separated by | symbol", "wgn-plugin"); ?>'>{categories}</option>
				</optgroup>
				<?php if ($is_wc_active) {
					?>
					<optgroup label="WooCommerc Tags">
						<option title='<?php echo __("The width of this product", "wgn-plugin"); ?>'>{width}</option>
						<option title='<?php echo __("The length of this product", "wgn-plugin"); ?>'>{length}</option>
						<option title='<?php echo __("The height of this product", "wgn-plugin"); ?>'>{height}</option>
						<option title='<?php echo __("The weight of this product", "wgn-plugin"); ?>'>{weight}</option>
						<option title='<?php echo __("The price of this product", "wgn-plugin"); ?>'>{price}</option>
						<option title='<?php echo __("The regular price of this product", "wgn-plugin"); ?>'>{regular_price}</option>
						<option title='<?php echo __("The sale price of this product", "wgn-plugin"); ?>'>{sale_price}</option>
						<option title='<?php echo __("The SKU (Stock Keeping Unit) of this product", "wgn-plugin"); ?>'>{sku}</option>
						<option title='<?php echo __("The stock amount of this product", "wgn-plugin"); ?>'>{stock}</option>
						<option title='<?php echo __("Is this product downloadable? (Yes or No)", "wgn-plugin"); ?>'>{downloadable}</option>
						<option title='<?php echo __("Is this product virtual? (Yes or No)", "wgn-plugin"); ?>'>{virtual}</option>
						<option title='<?php echo __("Is this product sold individually? (Yes or No)", "wgn-plugin"); ?>'>{sold_individually}</option>
						<option title='<?php echo __("The tax status of this product", "wgn-plugin"); ?>'>{tax_status}</option>
						<option title='<?php echo __("The tax class of this product", "wgn-plugin"); ?>'>{tax_class}</option>
						<option title='<?php echo __("The stock status of this product", "wgn-plugin"); ?>'>{stock_status}</option>
						<option title='<?php echo __("Whether or not backorders allowed? ", "wgn-plugin"); ?>'>{backorders}</option>
						<option title='<?php echo __("Is this a featured product? (Yes or No)", "wgn-plugin"); ?>'>{featured}</option>
						<option title='<?php echo __("Is this product visible? (Yes or No)", "wgn-plugin"); ?>'>{visibility}</option>
						<?php } ?>
					</optgroup>
				</select>
				<select id="emoji-select" class="wp-exclude-emoji">
				<option selected="true" style="display:none;"><?php echo __("Emoji...", "wgn-plugin"); ?></option>
			...
			<div id="wgn_box_container">
				<textarea id="wgn_channel_pattern" name="wgn_channel_pattern" dir="auto"><?php echo $cp ?></textarea>
				<div id="output" dir="auto"><?php echo $cp ?></div>
			</div>
			<div class="wgn-radio-group">
				<input type="radio" name="wgn_send_thumb" id="wgn-send-thumb-0" <?php echo ($s==0)?'checked=checked':'' ?> value="0">
				<label for="wgn-send-thumb-0"><?php echo __("Don't send featured image", "wgn-plugin"); ?></label>
				<br>
				<input type="radio" name="wgn_send_thumb" id="wgn-send-thumb-1" <?php echo ($s==1)?'checked=checked':'' ?> value="1">
				<label for="wgn-send-thumb-1"><?php echo __("Send featured image", "wgn-plugin"); ?></label>
				<br>
				<?php
				...
				if ($hook->base == 'post'){
				?>
				<input type="radio" name="wgn_send_thumb" id="wgn-send-thumb-2" <?php echo ($s==2)?'checked=checked':'' ?> value="2">
				<label for="wgn-send-thumb-2"><?php echo __("Send custom image", "wgn-plugin"); ?></label>
				<div class="wgn-img-container">
					<?php if ( $wgn_have_img ) : ?>
						<img src="<?php echo $wgn_img_src[0] ?>" alt="" style="max-width:100%;" />
					<?php endif; ?>
				</div>
				<!-- wgn add & remove image links -->
				<p id="wgn-upload-link" class="hide-if-no-js <?php if ($s != 2) { echo 'hidden'; } ?>">
					<a class="upload-custom-img <?php if ( $wgn_have_img  ) { echo 'hidden'; } ?>" 
						href="<?php echo $wgn_img_upload_link ?>">
						<?php _e('Set custom image') ?>
					</a>
					<a class="delete-custom-img <?php if ( ! $wgn_have_img  ) { echo 'hidden'; } ?>" 
						href="#">
						<?php _e('Remove this image') ?>
					</a>
				</p>
				<!-- A hidden input to set and post the chosen image id -->
				<input class="wgn-img-id" name="wgn_img_id" type="hidden" value="<?php echo esc_attr( $wgn_img_id ); ?>" />
				<?php } #end if ?>
			</div>
		</td>
	</tr>
	...
	<tr>
		<th scope="row">
			<h3><?php echo __("Send file", "wgn-plugin"); ?></h3>
		</th>
		<td>
			<p class="howto">
				<?php 
				echo __("Select the file that you want to send with this post.", "wgn-plugin");
				?>
			</p>
			<div class="wgn-file-container">
				<div id="wgn-file-icon">
					<?php if ( $wgn_have_file ) : ?>
						<a href="#" title="<?php echo __("Edit file", "wgn-plugin") ?>"><img src="<?php echo $attachment['icon'] ?>" alt="" style="max-width:100%;" /></a>
					<?php endif; ?>
				</div>
				<div id="wgn-file-details">
					<?php if ( $wgn_have_file ) : ?>
			...
			<!-- wgn add & remove image links -->
			<p id="wgn-upload-link" class="hide-if-no-js">
				<a class="upload-custom-file <?php if ( $wgn_have_file  ) { echo 'hidden'; } ?>" 
					href="<?php echo $wgn_file_upload_link ?>">
					<?php echo __('Set custom file', 'wgn-plugin') ?>
				</a>
				<a class="delete-custom-file <?php if ( ! $wgn_have_file  ) { echo 'hidden'; } ?>" 
					href="#">
					<?php echo __('Remove this file', 'wgn-plugin') ?>
				</a>
			</p>
			<!-- A hidden input to set and post the chosen file id -->
			<input class="wgn-file-id" name="wgn_file_id" type="hidden" value="<?php echo esc_attr( $wgn_file_id ); ?>" />
		</td>
	</tr>
	<?php } #end if ?>